describe('Should test Service Articles', () => {
  let articles;
  let $http;
  ...
  beforeEach(() => {
    ...
    angular.mock.inject((Articles, $httpBackend) => {
      articles = Articles;
      $http = $httpBackend;
    });
    $http.whenGET(`${urlBackend}/query/rest/articles/numPage/0/nbItem/10`).respond(200, arrayArticles);
    $http.whenGET(`${urlBackend}/query/rest/articles/numPage/0/nbItem/2`).respond(200, arrayArticles);
    $http.whenGET(`${urlBackend}/query/rest/article/5`).respond(200, arrayArticles[0]);
    $http.whenGET(`${urlBackend}/query/rest/articles/count`).respond(200, arrayArticles.length);
  });
  ...
  it('Should return articles', () => {
    $http.expectGET(`${urlBackend}/query/rest/articles/numPage/0/nbItem/10`);
    articles.getArticles().then(data => {
      expect(data).toEqual(arrayArticles);
    });
    $http.flush();
  });

  it('Should return 10 firt articles', () => {
    $http.expectGET(`${urlBackend}/query/rest/articles/numPage/0/nbItem/10`);
    articles.getArticles(0).then(data => {
      expect(data).toEqual(arrayArticles);
    });
    $http.flush();
  });

  it('Should return 2 firt articles', () => {
    $http.expectGET(`${urlBackend}/query/rest/articles/numPage/0/nbItem/2`);
    articles.getArticles(0, 2).then(data => {
      expect(data).toEqual(arrayArticles);
    });
    $http.flush();
  });

  it('Should return 1 article by his Id', () => {
    $http.expectGET(`${urlBackend}/query/rest/article/5`);
    articles.getArticle(5).then(data => {
      expect(data).toEqual(arrayArticles[0]);
    });
    $http.flush();
  });

  it('Should save a new article and return his Id', () => {
    let article = {
    ...
    $http.expectPOST(`${urlBackend}/command/rest/article/`, article).respond(201);
    articles.saveArticle(article);
    $http.flush();
  });

  it('Should return count of articles', () => {
    $http.expectGET(`${urlBackend}/query/rest/articles/count`);
    articles.countArticles().then(data => {
      expect(data).toEqual(arrayArticles.length);
    });
    $http.flush();
  });
});
